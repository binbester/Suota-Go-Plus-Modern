...
{
    /// <summary>
    /// Dummy GATT callback for connection priority request.
    /// </summary>
    internal class DummyGattCallback : Android.Bluetooth.BluetoothGattCallback
    {
    }
    ...
    internal class BleManager : IBleManager
    {
        ...
        public async Task ConnectDevice(GoPlus device)
        {
            ...
            for (int i = 0; i < Constants.RetryCount; i++)
            {
                try
                {
                    ...
                    // Small delay after connection before starting operations
        ...
        /// <param name="charUuid"></param>
        /// <param name="value"></param>
        /// <param name="noResponse">Use write without response</param>
        /// <param name="skipPreDelay">Skip the pre-write delay (for rapid data transfers)</param>
        public async Task WriteCharacteristic(GoPlus device, Guid charUuid, byte[] value, bool noResponse = false, bool skipPreDelay = false)
        {
            if (device == null)
                throw new ArgumentNullException("device");
            ...
            // Wait a bit before writing (skip for rapid data transfers)
            if (!skipPreDelay)
            {
                await Task.Delay(Constants.DelayMS);
            }
